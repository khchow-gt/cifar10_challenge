...
def evaluate_checkpoint(filename):
    with tf.Session() as sess:
        ...
        for ibatch in range(num_batches):
            ...
            cur_corr_nat, cur_xent_nat = sess.run([model.num_correct, model.xent], feed_dict=dict_nat)
            cur_corr_adv, cur_xent_adv = sess.run([model.num_correct, model.xent], feed_dict=dict_adv)
...
while True:
    ...
    # Case 2: Previously unseen checkpoint
        ...
        print('\nCheckpoint {}, evaluating ...   ({})'.format(cur_checkpoint, datetime.now()))
        sys.stdout.flush()
    ...
    # Case 3: Previously evaluated checkpoint
        ...
        if not already_seen_state:
            print('Waiting for the next checkpoint ...   ({})   '.format(datetime.now()), end='')
            already_seen_state = True
        else:
            print('.', end='')
        sys.stdout.flush()
